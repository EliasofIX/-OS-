...
static int begin_device(uintptr_t base, uint32_t accepted_low) {
    mmio_write(base, V_STATUS, 0);
    if (mmio_read(base, V_STATUS) != 0) return 0;
    ...
    if (!(mmio_read(base, V_DEVICE_FEATURES) & 1U)) return 0;

    mmio_write(base, V_DRIVER_FEATURES_SEL, 0);
    mmio_write(base, V_DRIVER_FEATURES, accepted_low);
    mmio_write(base, V_DRIVER_FEATURES_SEL, 1);
    mmio_write(base, V_DRIVER_FEATURES, 1);
    mmio_write(base, V_STATUS,
               STATUS_ACKNOWLEDGE | STATUS_DRIVER | STATUS_FEATURES_OK);
    if (!(mmio_read(base, V_STATUS) & STATUS_FEATURES_OK)) return 0;
    return 1;
}
...
static int init_input(uintptr_t base) {
    if (input_count >= 2 || !begin_device(base, 0)) return 0;
    ...
    device->queue_size = maximum < INPUT_QUEUE_SIZE ? (uint16_t)maximum :
                                                    INPUT_QUEUE_SIZE;
    ...
    ++input_count;
    uart_puts(device->kind == INPUT_TABLET ?
...
static int init_block(uintptr_t base) {
    ...
    uint32_t offered_low = mmio_read(base, V_DEVICE_FEATURES);
    if (offered_low & (1U << 5)) return 0;
    uint32_t accepted_low = offered_low & (1U << 9);
    if (!begin_device(base, accepted_low)) return 0;

    zero_memory(&block, sizeof(block));
    block.base = base;
    block.flush = !!(accepted_low & (1U << 9));
    mmio_write(base, V_QUEUE_SEL, 0);
    ...
    address_registers(base, V_QUEUE_DESC_LOW, block.descriptors);
    address_registers(base, V_QUEUE_AVAIL_LOW, &block.available);
    address_registers(base, V_QUEUE_USED_LOW, &block.used);
    block.available.flags = 1;
    mmio_write(base, V_QUEUE_READY, 1);
    ...
    if (block.ready) uart_puts("virtio: persistent document disk ready\n");
    return block.ready;
}
...
void virtio_init(void) {
    ...
    for (int slot = 0; slot < VIRTIO_SLOTS; ++slot) {
        uintptr_t base = VIRTIO_BASE + (uintptr_t)slot * VIRTIO_STRIDE;
        if (mmio_read(base, V_MAGIC) != 0x74726976U ||
            mmio_read(base, V_VERSION) != 2) continue;
        uint32_t id = mmio_read(base, V_DEVICE_ID);
        if (id == DEVICE_INPUT) (void)init_input(base);
        else if (id == DEVICE_BLOCK && !block.ready) (void)init_block(base);
    }
}
    ...
    for (int index = 0; index < input_count; ++index) {
        ...
        if (device->kind == INPUT_TABLET && packet.type == EV_ABS) {
            if (packet.code == ABS_X)
                device->pointer_x = (int)((uint64_t)packet.value *
                                          (SCREEN_WIDTH - 1) / 32767U);
            else if (packet.code == ABS_Y)
                device->pointer_y = (int)((uint64_t)packet.value *
                                          (SCREEN_HEIGHT - 1) / 32767U);
            else continue;
...
static int block_request(uint32_t type, uint64_t sector, void *buffer) {
    if (!block.ready || sector >= block.capacity) return 0;
    block.request.type = type;
    block.request.reserved = 0;
    block.request.sector = sector;
    block.request_status = 0xff;

    block.descriptors[0] = (struct virtq_desc){
        (uint64_t)(uintptr_t)&block.request, sizeof(block.request), DESC_NEXT, 1};
    if (type == 4) {
        block.descriptors[1] = (struct virtq_desc){
            (uint64_t)(uintptr_t)&block.request_status, 1, DESC_WRITE, 0};
    } else {
        if (type == 1) copy_memory(block.sector_buffer, buffer, 512);
        block.descriptors[1] = (struct virtq_desc){
            (uint64_t)(uintptr_t)block.sector_buffer, 512,
            (uint16_t)(DESC_NEXT | (type == 0 ? DESC_WRITE : 0)), 2};
        block.descriptors[2] = (struct virtq_desc){
            (uint64_t)(uintptr_t)&block.request_status, 1, DESC_WRITE, 0};
    }

    block.available.ring[block.available_index % BLOCK_QUEUE_SIZE] = 0;
    __asm__ volatile("dmb oshst" ::: "memory");
    block.available.index = ++block.available_index;
    __asm__ volatile("dmb oshst" ::: "memory");
    mmio_write(block.base, V_QUEUE_NOTIFY, 0);

    uint32_t timeout = 10000000;
    while (block.used.index == block.used_index && timeout--) {}
    if (!timeout) return 0;
    __asm__ volatile("dmb oshld" ::: "memory");
    ++block.used_index;
    if (block.request_status != 0) return 0;
    if (type == 0) copy_memory(buffer, block.sector_buffer, 512);
    return 1;
}
...
static void flush_disk(void) {
    if (block.flush) (void)block_request(4, 0, 0);
}
...
static int valid_commit(const struct document_commit *commit) {
    if (commit->magic != DOCUMENT_MAGIC ||
        ...
        commit->length > DOCUMENT_CAPACITY) return 0;
...
static int read_slot(int slot, struct document_commit *commit, char *text) {
    ...
    if (!read_sector(first, commit) || !valid_commit(commit)) return 0;
    for (int sector = 0; sector < 4; ++sector)
        if (!read_sector(first + 1 + sector, text + sector * 512)) return 0;
    return checksum(text, commit->length) == commit->checksum;
}

int storage_available(void) { return block.ready; }

int storage_load_document(char *text, size_t capacity, size_t *length) {
    if (!block.ready || capacity < DOCUMENT_CAPACITY) return 0;
    struct document_commit first;
    struct document_commit second;
    static char first_text[DOCUMENT_CAPACITY];
    static char second_text[DOCUMENT_CAPACITY];
    int valid_first = read_slot(0, &first, first_text);
    int valid_second = read_slot(1, &second, second_text);
    if (!valid_first && !valid_second) return 0;
    int use_second = valid_second &&
                     (!valid_first || second.generation > first.generation);
    struct document_commit *chosen = use_second ? &second : &first;
    copy_memory(text, use_second ? second_text : first_text, chosen->length);
    *length = chosen->length;
    return 1;
}

int storage_save_document(const char *text, size_t length) {
    if (!block.ready || length > DOCUMENT_CAPACITY) return 0;
    struct document_commit first;
    struct document_commit second;
    static char scratch[DOCUMENT_CAPACITY];
    int valid_first = read_slot(0, &first, scratch);
    int valid_second = read_slot(1, &second, scratch);
    uint64_t generation = 1;
    int slot = 0;
    if (valid_first || valid_second) {
        if (valid_second && (!valid_first || second.generation > first.generation)) {
    ...
    for (int sector = 0; sector < 4; ++sector)
        if (!write_sector(first_sector + 1 + sector,
                          scratch + sector * 512)) return 0;
    flush_disk();
    ...
    commit.metadata_checksum =
        checksum(&commit, offsetof(struct document_commit, metadata_checksum));
    if (!write_sector(first_sector, &commit)) return 0;
    flush_disk();
    return 1;
}
